...
export const TOOL_CONFIGS: Record<string, ToolDisplayConfig> = {
  ...
  Read: {
    input: {
      type: 'one-line',
      label: 'Read',
      getValue: (input) => displayFilePath(input.file_path) || '',
      action: 'open-file',
    },
    result: { hidden: true },
  },
  Edit: {
    input: {
      type: 'collapsible',
      title: (input) => displayFilePath(input.file_path) || 'file',
      defaultOpen: false,
      contentType: 'diff',
  ...
  TodoWrite: {
    input: {
      ...
      getContentProps: (input) => ({ todos: input.todos }),
    },
    result: {
      type: 'collapsible',
    ...
    result: {
      ...
      getContentProps: (result) => {
        try {
          ...
          return { content: parsed.plan?.replace(/\\n/g, '\n') || parsed.plan };
        } catch {
          return { content: '' };
        }
      },
...
function displayFilePath(p?: string): string {
  if (!p) return '';
  const normalized = p.replace(/\\/g, '/');
  const idx = normalized.indexOf('.claude-uploads');
